...
class OTB_DefaultVariableCategoryTable extends React.Component {
   ...
   render() {
       ...
       if (true){ 
           return(
               <OTB_Table className="o-t-b_-default-variable-category-table"  titles={['Variable Name', 'Variable Description', 'Variable Handler', 'Variable Value']}>
{                  self.props.commandVariableCategory && self.props.commandVariableCategory.get('categoryCommandVariables') ? self.props.commandVariableCategory.get('categoryCommandVariables').map(function(item, index) {
                      return(<OTB_CommandVariableRow commandVariable={item} index={index}  key={item.id || index} user={self.props.user} />);
                  }) : ''}
